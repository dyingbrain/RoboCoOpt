...
class GA:
    ...
    def __init__(self, fitness_func, soln_generator,
                 ...
                 crossover_fn=None, mutation_fn=None,
        ...
        self.crossover_fn = crossover_fn if crossover_fn is not None else "single_point"
        self.mutation_fn = mutation_fn if mutation_fn is not None else "random"
    ...
    def generate_init_state(self, ret=None):
        ...
        for i in range(len(ret) // 5, self.K):
            ...
            if state == GA.NOT_USED:
                ...
                ret += [-1.0, -1.0, random.uniform(-self.B, self.B), random.uniform(-self.B, self.B),
                        float(state)]
            elif i == 0:
                ret += [-1.0, 1, 0.0, 0.0, float(state)]
            else:
    ...
    def generate_gen_space(self):
        gene_space = [
            [-1], [self.B / 10, self.B], [0.0], [0.0], [0.0, 1.0]  # Motor
        ]

        gene_space += [
            [-1], [-1], [-self.B, self.B], [-self.B, self.B], [1.0]  # First node
        ]

        for i in range(2, self.K):
            gene_space += [
                [j for j in range(-1, i)], [j for j in range(-1, i)],
                [0, self.maxDist], [0, self.maxDist], [0.0, 1.0]  # Other nodes
            ]
    ...
    def run(self):
        self.ga_instance = pygad.GA(
            ...
            crossover_type=self.crossover_fn,  # Use custom or default crossover
            mutation_type=self.mutation_fn,  # Use custom or default mutation
